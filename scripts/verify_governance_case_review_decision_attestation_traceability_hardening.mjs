...
await import("./verify_governance_case_review_decision_attestation_traceability_boundary.mjs");

{
  ...
  const validation =
    validateGovernanceCaseReviewDecisionAttestationTraceabilityContract(contract);
  if (!validation.ok) {
    throw new Error(
      `review decision attestation traceability hardening contract invalid: ${validation.errors.join("; ")}`
    );
  }
  for (const field of [
    ...
    if (contract[field] !== true) {
      throw new Error(
        `review decision attestation traceability hardening contract drifted: ${field}`
      );
    }
  }
  for (const field of [
  ...
  if (
    contract.kind !== GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_CONTRACT_KIND ||
    contract.version !== GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_CONTRACT_VERSION ||
    contract.boundary !== GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_CONTRACT_BOUNDARY
  ) {
    throw new Error("review decision attestation traceability hardening contract envelope drifted");
  }
}

{
  ...
  }, "unique_current_view_required", "review decision attestation traceability must reject non-unique current attestation view");
}

{
  const fixture = buildFixture();
  assertRejected(() => {
    const broken = cloneJson(fixture.explanation);
  ...
  }, "selection_receipt_aligned", "review decision attestation traceability must reject missing explanation-basis linkage");
}

{
  const fixture = buildFixture();
  assertRejected(() => {
    const broken = cloneJson(fixture.receipt);
    broken.governance_case_review_decision_attestation_receipt.receipt_context.receipt_basis.attestation_explanation_aligned =
      false;
    return buildGovernanceCaseReviewDecisionAttestationTraceability({
      governanceCaseReviewDecisionAttestationProfile: fixture.attestation,
      governanceCaseReviewDecisionAttestationExplanationProfile:
        fixture.explanation,
      governanceCaseReviewDecisionAttestationReceiptProfile: broken,
    });
  }, "attestation_explanation_aligned", "review decision attestation traceability must reject missing receipt-basis linkage");
}

{
  ...
  assertRejected(() => {
    const broken = cloneJson(fixture.receipt);
    broken.governance_case_review_decision_attestation_receipt.preserved_semantics.authority_source_enabled =
      true;
    return buildGovernanceCaseReviewDecisionAttestationTraceability({
      governanceCaseReviewDecisionAttestationProfile: fixture.attestation,
      governanceCaseReviewDecisionAttestationExplanationProfile:
  ...
  }, "authority_source_enabled", "review decision attestation traceability must reject authoritative supporting receipt semantics");
}

{
  ...
  assertRejected(() => {
    ...
    mismatched.attestation_traceability_profile_ref.case_id = "case-other";
    return consumeGovernanceCaseReviewDecisionAttestationTraceability({
      governanceCaseReviewDecisionAttestationTraceabilityProfile: profile,
      governanceCaseReviewDecisionAttestationTraceabilityContract: mismatched,
    });
  }, "aligned", "review decision attestation traceability consumer must reject case mismatch");
}

if (
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.non_executing !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.unique_current_attestation_view_required !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.attestation_explanation_alignment_required !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.attestation_receipt_alignment_required !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.complete_supporting_linkage_required !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.linkage_integrity_preserved !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.non_authoritative_support_only !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.traceability_basis_support_only !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.attestation_trace_platform !== false ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.observability_platform_behavior !== false ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_TRACEABILITY_SURFACE_MAP
    .governance_case_review_decision_attestation_traceability.traceability_platform_behavior !== false
) {
  throw new Error("review decision attestation traceability hardening surface drifted");
}

process.stdout.write(
  "governance case review decision attestation traceability hardening verified\n"
);
