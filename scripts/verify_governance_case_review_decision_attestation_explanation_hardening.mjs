...

{
  ...
  const validation = validateGovernanceCaseReviewDecisionAttestationExplanationContract(contract);
  if (!validation.ok) {
    throw new Error(`review decision attestation explanation hardening contract invalid: ${validation.errors.join("; ")}`);
  }
  for (const field of [
    ...
    if (contract[field] !== true) {
      throw new Error(`review decision attestation explanation hardening contract drifted: ${field}`);
    }
  }
  for (const field of ["permit_lane_consumption", "audit_path_dependency", "main_path_takeover"]) {
...

{
  ...
  }, "unique_current_view_required", "review decision attestation explanation must reject non-unique current attestation view");
}

{
  ...
  assertRejected(() => {
    const broken = cloneJson(fixture.attestationProfile);
    broken.governance_case_review_decision_attestation.attestation_context.attestation_basis.selection_receipt_linked = false;
    return buildGovernanceCaseReviewDecisionAttestationExplanation({
      governanceCaseReviewDecisionAttestationProfile: broken,
      governanceCaseReviewDecisionSelectionExplanationFinalAcceptanceBoundary: fixture.selectionExplanationFinalAcceptance,
      governanceCaseReviewDecisionSelectionReceiptFinalAcceptanceBoundary: fixture.selectionReceiptFinalAcceptance,
  ...
  }, "selection_receipt_linked", "review decision attestation explanation must reject missing attestation-basis linkage");
}

{
  ...
  assertRejected(() => {
    ...
    broken.governance_case_review_decision_applicability_explanation.preserved_semantics.authority_source_enabled = true;
    return buildGovernanceCaseReviewDecisionAttestationExplanation({
      governanceCaseReviewDecisionAttestationProfile: fixture.attestationProfile,
      governanceCaseReviewDecisionSelectionExplanationFinalAcceptanceBoundary: fixture.selectionExplanationFinalAcceptance,
      governanceCaseReviewDecisionSelectionReceiptFinalAcceptanceBoundary: fixture.selectionReceiptFinalAcceptance,
  ...
  }, "authority_source_enabled", "review decision attestation explanation must reject authoritative supporting explanation semantics");
}

{
  ...
  assertRejected(() => {
    ...
    mismatched.attestation_explanation_profile_ref.case_id = "case-other";
    return consumeGovernanceCaseReviewDecisionAttestationExplanation({
      governanceCaseReviewDecisionAttestationExplanationProfile: profile,
      governanceCaseReviewDecisionAttestationExplanationContract: mismatched,
    });
  }, "aligned", "review decision attestation explanation consumer must reject case mismatch");
}

if (
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_attestation_explanation.non_executing !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_attestation_explanation.unique_current_attestation_view_required !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_ATTESTATION_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_attestation_explanation.complete_supporting_linkage_required !== true
) {
  throw new Error("review decision attestation explanation hardening surface drifted");
}

process.stdout.write(
  "governance case review decision attestation explanation hardening verified\n"
);
