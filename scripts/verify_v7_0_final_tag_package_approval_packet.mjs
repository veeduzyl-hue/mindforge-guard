...
const REQUIRED_DOC_PATH = "docs/release/v7_0_final_tag_package_approval_packet.md";
...
const REQUIRED_WAVE_REFERENCES = [
...
function main() {
  ...
  for (const phrase of REQUIRED_WAVE_REFERENCES) {
    expect(docText.includes(phrase), `required prior wave reference missing: ${phrase}`);
  }

  expect(docText.includes("candidate tag: `v7.0.0`"), "candidate tag v7.0.0 must appear");
  expect(docText.includes("candidate tag remains under consideration only and has not been created"), "candidate tag wording must not imply tag creation");

  for (const source of REQUIRED_SOURCE_REFERENCES) {
    expect(docText.includes(source), `required release candidate source reference missing: ${source}`);
  }
  ...
  console.log("PASS: v7.0 final tag/package approval packet verified.");
}
