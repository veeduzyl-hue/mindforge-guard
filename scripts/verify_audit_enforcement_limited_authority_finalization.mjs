...
const outA = path.join(
  os.tmpdir(),
  "mindforge-guard-limited-enforcement-authority-finalization-a.json"
);
const outB = path.join(
  os.tmpdir(),
  "mindforge-guard-limited-enforcement-authority-finalization-b.json"
);
...
if (
  ...
  throw new Error("limited enforcement authority finalization changed audit main output");
}
if (baseline.exitCode !== pilotA.exitCode) {
  throw new Error("limited enforcement authority finalization changed audit exit code");
}

permit.assertValidLimitedEnforcementAuthorityFinalization(
...
if (serializedA !== expectedA || serializedB !== expectedB) {
  throw new Error(
    "limited enforcement authority finalization serialized output drifted from finalized contract"
  );
}
if (serializedA !== serializedB) {
  throw new Error(
    "limited enforcement authority finalization serialized output is not stable across repeated runs"
  );
}
...
if (
  permit.LIMITED_ENFORCEMENT_AUTHORITY_FINAL_ACCEPTANCE_BOUNDARY !==
  "final_explicit_opt_in_recommendation_only_authority_sidecar"
) {
  throw new Error("limited enforcement authority final acceptance boundary drifted");
}
if (payload.authority_scope !== "review_gate_deny_exit_recommendation_only") {
  throw new Error("limited enforcement authority finalization scope drifted");
}
if (payload.current_audit_exit_code !== null) {
  throw new Error(
    "limited enforcement authority finalization must preserve null current_audit_exit_code"
  );
}
for (const exportName of permit.LIMITED_ENFORCEMENT_AUTHORITY_FINAL_EXPORT_SET) {
  if (!(exportName in permit)) {
    throw new Error(
      `limited enforcement authority finalization export missing from permit index: ${exportName}`
    );
  }
}

process.stdout.write("audit limited enforcement authority finalization verified\n");
