...
function createTempFixtures(repoRoot) {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mf-guard-guardrail-preview-"));
  const baseFixture = JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "fixtures", "guardrail_mapping", "symbolic-guardrail-mapping.cases.valid.json"),
      "utf8"
    )
  );
...
async function main() {
  ...
  expect(firstPayload.schema_version === "guard.symbolic_guardrail_mapping.v1", "schema version mismatch");
  expect(firstPayload.preview === true, "preview flag mismatch");
  expect(firstPayload.mapping_only === true, "mapping_only flag mismatch");
  ...
  expect(Array.isArray(firstPayload.requirements) && firstPayload.requirements.length === 5, "requirements length mismatch");
  expect(Array.isArray(firstPayload.mappings) && firstPayload.mappings.length === 5, "mappings length mismatch");
  expect(Array.isArray(firstPayload.unmapped_requirements), "unmapped_requirements must be an array");

  const statuses = new Set(firstPayload.mappings.map((entry) => entry.mapping_status));
  for (const status of ["mapped", "partially_mapped", "unmapped", "out_of_scope", "unknown"]) {
    expect(statuses.has(status), `mapping status ${status} must be present`);
  }

  expect(firstPayload.input_summary.case_count === 5, "case_count mismatch");
  expect(firstPayload.input_summary.requirement_count === 5, "requirement_count mismatch");
  ...
  expect(boundary.does_not_evaluate_guardrail_result === true, "does_not_evaluate_guardrail_result mismatch");
  ...
  const missingFixture = await runGuard({
    argv: ["guardrail", "map", "--preview", "--json"],
  });
  ...
  try {
    ...
    expect(
      parseJsonOutput(missingFixtureResult, "missing fixture").error.kind === "guardrail_mapping_preview_fixture_missing",
    ...
    expect(
      parseJsonOutput(invalidJsonResult, "invalid JSON fixture").error.kind === "guardrail_mapping_preview_fixture_invalid_json",
  ...
  process.stdout.write("symbolic guardrail mapping preview verified\n");
}
