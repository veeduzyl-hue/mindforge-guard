  ...
  return buildPolicyPermitBridgeContract({
    canonicalActionArtifact: {
      canonical_action_hash:
        "sha256:6666aaaabbbb7777cccc8888dddd9999eeee0000ffff1111aaaa2222bbbb3333",
...
await import("./verify_governance_case_review_decision_applicability_boundary.mjs");

{
  const fixture = buildSelectedFixture();
  const selectionBefore = stableStringify(fixture.selectionProfile);
  const applicabilityBefore = stableStringify(fixture.applicabilityProfile);
  ...
  const consumed = consumeGovernanceCaseReviewDecisionApplicabilityExplanation({
    governanceCaseReviewDecisionApplicabilityExplanationProfile:
      explanationProfile,
  ...
  const payload =
    explanationProfile.governance_case_review_decision_applicability_explanation;

  if (stableStringify(fixture.selectionProfile) !== selectionBefore) {
    throw new Error(
      "review decision applicability explanation must not affect current selection"
    );
  }
  if (stableStringify(fixture.applicabilityProfile) !== applicabilityBefore) {
    throw new Error(
      "review decision applicability explanation must not affect applicability profile"
    );
  }
  if (
    validateGovernanceCaseReviewDecisionApplicabilityExplanationProfile(
    ...
    throw new Error(
      "review decision applicability explanation profile validation failed"
    );
  }
  if (
    validateGovernanceCaseReviewDecisionApplicabilityExplanationContract(
    ...
    throw new Error(
      "review decision applicability explanation contract validation failed"
    );
  }
  if (
    ...
    throw new Error(
      "review decision applicability explanation profile envelope drifted"
    );
  }
  if (
    payload.explanation_context.explanation_status !==
      GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_STATUS_AVAILABLE ||
    JSON.stringify(payload.explanation_context.explanation_reason_codes) !==
      JSON.stringify([
        GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_REASON_SELECTED_CURRENT_REVIEW_DECISION_APPLICABLE,
        GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_REASON_APPLICABILITY_PROFILE_ALIGNED,
        GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_REASON_CURRENT_SELECTION_FINAL_ACCEPTANCE_READY,
    ...
    throw new Error(
      "review decision applicability explanation bounded reasons drifted"
    );
  }
  for (const field of [
    ...
    if (payload.validation_exports[field] !== true) {
      throw new Error(
        `review decision applicability explanation validation export drifted: ${field}`
      );
    }
  }
  for (const field of [
    ...
    if (payload.preserved_semantics[field] !== true) {
      throw new Error(
        `review decision applicability explanation preserved semantic drifted: ${field}`
      );
    }
  }
  for (const field of [
  ...
  if (
    ...
    throw new Error(
      "review decision applicability explanation contract envelope drifted"
    );
  }
  for (const field of [
    ...
    if (explanationContract[field] !== true) {
      throw new Error(
        `review decision applicability explanation contract drifted: ${field}`
      );
    }
  }
  for (const field of [
    ...
    if (explanationContract[field] !== false) {
      throw new Error(
        `review decision applicability explanation contract drifted: ${field}`
      );
    }
  ...
  if (
    ...
    throw new Error(
      "review decision applicability explanation consumed summary drifted"
    );
  }
}
  ...
  assertRejected(
    () => {
      ...
      unsupported.governance_case_review_decision_current_selection_final_acceptance.acceptance_scope.current_selection_summary_current_review_decision =
        null;
      return buildGovernanceCaseReviewDecisionApplicabilityExplanation({
        governanceCaseReviewDecisionCurrentSelectionFinalAcceptanceBoundary:
          unsupported,
        governanceCaseReviewDecisionApplicabilityProfile:
          fixture.applicabilityProfile,
    ...
    "only supports selected current selection",
    "review decision applicability explanation must reject non-selected state"
  );
}

{
  ...
  assertRejected(
    () => {
      const mismatchedApplicability = cloneJson(fixture.applicabilityProfile);
      mismatchedApplicability.governance_case_review_decision_applicability.applicability_ref.current_review_decision_id =
        "review-other";
      return buildGovernanceCaseReviewDecisionApplicabilityExplanation({
        governanceCaseReviewDecisionCurrentSelectionFinalAcceptanceBoundary:
          fixture.selectionFinalAcceptance,
        governanceCaseReviewDecisionApplicabilityProfile: mismatchedApplicability,
        governanceCaseReviewDecisionProfiles: [fixture.current, fixture.previous],
      });
    },
    "applicability profile must remain aligned",
    "review decision applicability explanation must reject applicability mismatch"
  );
}

{
  ...
  assertRejected(
    ...
    "review decision applicability explanation must reject missing selected review decision profile"
  );
}

{
  ...
  assertRejected(
    ...
    "review decision applicability explanation must reject superseded selected review decision"
  );
}

{
  ...
  assertRejected(
    () => {
      const mismatched = cloneJson(explanationContract);
      mismatched.current_review_decision_id = "review-other";
      return consumeGovernanceCaseReviewDecisionApplicabilityExplanation({
        governanceCaseReviewDecisionApplicabilityExplanationProfile:
          explanationProfile,
        governanceCaseReviewDecisionApplicabilityExplanationContract: mismatched,
      });
    },
    "profile and contract must remain aligned",
    "review decision applicability explanation consumer must reject contract mismatch"
  );
}
...
if (
  ...
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.additive_only !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.default_off !== true ||
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.judgment_source_enabled !== false ||
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.authority_source_enabled !== false ||
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.audit_path_dependency !== false ||
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.main_path_takeover !== false ||
  GOVERNANCE_CASE_REVIEW_DECISION_APPLICABILITY_EXPLANATION_SURFACE_MAP
    .governance_case_review_decision_applicability_explanation.executing !== false
) {
  throw new Error(
    "review decision applicability explanation export surface drifted"
  );
}
...
for (const exportName of [
  ...
  if (!(exportName in permitExports)) {
    throw new Error(
      `review decision applicability explanation permit export missing: ${exportName}`
    );
  }
}

process.stdout.write(
  "governance case review decision applicability explanation boundary verified\n"
);
