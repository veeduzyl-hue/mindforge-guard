...
const driftEventsPath = path.join(process.cwd(), ".mindforge", "drift", "events.jsonl");
const auditHistorySnapshot = captureAuditHistory(auditHistoryPath);
const driftEventsSnapshot = captureAuditHistory(driftEventsPath);
...
try {
  ...
  const allow = await runAuditWithHistoryBaseline({
    argv: [
      ".",
      "--staged",
      "--permit-gate",
  ...
  const deny = await runAuditWithHistoryBaseline({
    argv: [
      ".",
      "--staged",
      "--permit-gate",
