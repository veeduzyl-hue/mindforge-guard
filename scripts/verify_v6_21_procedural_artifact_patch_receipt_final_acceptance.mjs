...
const previewScriptPath = path.join(
  repoRoot,
  "scripts",
  "verify_v6_21_procedural_artifact_patch_receipt_preview.mjs"
);
const finalAcceptanceDocPath = path.join(
  repoRoot,
  "docs",
  "governance",
  "v6_21_procedural_artifact_patch_receipt_final_acceptance.md"
);
...
function expectSchemaShape(schema) {
  ...
  expectExactArray(
    schema.$defs.artifactSubjectType.enum,
    EXPECTED_ARTIFACT_SUBJECT_TYPES,
    "artifact_subject_type enum"
  );
...
function validateFixtureBundle(fixture, schema) {
  ...
  expect(
    fixture.schema_version === "guard.procedural_artifact_patch_receipt_fixture.v1",
...
function validatePreviewReceipt(receipt, schema, label) {
  ...
  expect(
    Array.isArray(receipt.consumed_receipt_refs) && receipt.consumed_receipt_refs.length >= 1,
    `${label}.consumed_receipt_refs must be a non-empty array`
  );
  receipt.consumed_receipt_refs.forEach((ref, index) => {
    validateConsumedReceiptRef(ref, schema, `${label}.consumed_receipt_refs[${index}]`);
  });
  expect(
    Array.isArray(receipt.procedural_findings) && receipt.procedural_findings.length >= 1,
    `${label}.procedural_findings must be a non-empty array`
  );
  receipt.procedural_findings.forEach((finding, index) => {
    validateProceduralFinding(finding, schema, `${label}.procedural_findings[${index}]`);
  });
...
function createTempFixtures(baseFixture) {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mf-guard-procedural-final-"));
...
async function main() {
  const previewStdout = runNodeScript(
    [previewScriptPath],
    "scripts/verify_v6_21_procedural_artifact_patch_receipt_preview.mjs"
  );
  ...
  if (previewTrimmed.startsWith("{")) {
    ...
    expect(
      previewStdout === "procedural artifact patch receipt preview verified\n",
  ...
  expectFileExists(finalAcceptanceDocPath, "v6.21 final acceptance doc");

  const previewScriptSource = fs.readFileSync(previewScriptPath, "utf8");
  expect(
    !previewScriptSource.includes("runGuard.mjs"),
    "v6.21 preview verifier must not require runGuard dispatch"
  );
  expect(
    !previewScriptSource.includes("guard procedure receipt --preview --json --fixture-file <file>"),
    "v6.21 preview verifier must not require a CLI command"
  );
  ...
  expect(
    ...
    "preview receipts must remain deterministic across repeated runs"
  );
  ...
  const observedPatchStatuses = [
    ...new Set(firstPayload.receipts.map((receipt) => receipt.declared_patch.patch_status)),
  ].sort();
  expectExactArray(observedPatchStatuses, [...EXPECTED_PATCH_STATUSES].sort(), "patch_status coverage");

  const observedArtifactSubjectTypes = [
    ...new Set(firstPayload.receipts.map((receipt) => receipt.artifact_subject.artifact_subject_type)),
  ].sort();
  expectExactArray(
  ...
  const observedFindingTypes = [
    ...new Set(
      firstPayload.receipts.flatMap((receipt) =>
        receipt.procedural_findings.map((finding) => finding.finding_type)
      )
    ),
  ].sort();
  ...
  firstPayload.receipts.forEach((receipt, index) => {
    ...
    expect(receipt.schema_version, `${label}.schema_version must exist`);
    expect(receipt.preview === true, `${label}.preview mismatch`);
    expect(receipt.receipt_only === true, `${label}.receipt_only mismatch`);
    expect(receipt.explanation_only === true, `${label}.explanation_only mismatch`);
    expect(receipt.execution_performed === false, `${label}.execution_performed mismatch`);
    expect(receipt.installation_performed === false, `${label}.installation_performed mismatch`);
    expect(receipt.permission_granted === false, `${label}.permission_granted mismatch`);
    expect(receipt.workflow_deployed === false, `${label}.workflow_deployed mismatch`);
    expect(
      receipt.agent_orchestration_performed === false,
      `${label}.agent_orchestration_performed mismatch`
    );
    expect(receipt.file_mutation_performed === false, `${label}.file_mutation_performed mismatch`);
    expect(receipt.enforcement_action === "none", `${label}.enforcement_action mismatch`);
    expect(receipt.blocking_effect === false, `${label}.blocking_effect mismatch`);
    expect(receipt.input_ref, `${label}.input_ref must exist`);
    expect(receipt.artifact_subject, `${label}.artifact_subject must exist`);
    expect(receipt.declared_patch, `${label}.declared_patch must exist`);
    expect(Array.isArray(receipt.consumed_receipt_refs), `${label}.consumed_receipt_refs must exist`);
    expect(Array.isArray(receipt.procedural_findings), `${label}.procedural_findings must exist`);
    expect(receipt.receipt_boundary, `${label}.receipt_boundary must exist`);
    expect(receipt.deterministic_hash, `${label}.deterministic_hash must exist`);
  });
  ...
  try {
    ...
    expect(
      parseJsonOutput(missingFixtureResult.stdout, "missing fixture error").error.kind ===
        "procedural_artifact_patch_receipt_fixture_missing",
    ...
    expect(
      parseJsonOutput(invalidJsonResult.stdout, "invalid fixture error").error.kind ===
        "procedural_artifact_patch_receipt_fixture_invalid_json",
    ...
    expect(
      parseJsonOutput(malformedResult.stdout, "malformed fixture error").error.kind ===
        "procedural_artifact_patch_receipt_contract_invalid",
  ...
  expect(!observedExitCodes.has(21), "v6.21 preview must not use exit 21");
  expect(!observedExitCodes.has(25), "v6.21 preview must not use exit 25");
  ...
  process.stdout.write("PASS verify_v6_21_procedural_artifact_patch_receipt_final_acceptance\n");
}
