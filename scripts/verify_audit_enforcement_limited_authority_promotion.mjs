...
const outA = path.join(
  os.tmpdir(),
  "mindforge-guard-limited-enforcement-authority-promotion-a.json"
);
const outB = path.join(
  os.tmpdir(),
  "mindforge-guard-limited-enforcement-authority-promotion-b.json"
);
...
if (JSON.stringify(normalizeAudit(baseline.audit)) !== JSON.stringify(normalizeAudit(pilotA.audit))) {
  throw new Error("limited enforcement authority promotion changed audit main output");
}
if (baseline.exitCode !== pilotA.exitCode) {
  throw new Error("limited enforcement authority promotion changed audit exit code");
}

permit.assertValidLimitedEnforcementAuthorityPromotion(
...
if (serializedA !== expectedA || serializedB !== expectedB) {
  throw new Error(
    "limited enforcement authority promotion serialized output drifted from promoted contract"
  );
}
if (serializedA !== serializedB) {
  throw new Error(
    "limited enforcement authority promotion serialized output is not stable across repeated runs"
  );
}
...
if (payload.authority_scope !== "review_gate_deny_exit_recommendation_only") {
  throw new Error("limited enforcement authority promotion scope drifted");
}
if (payload.current_audit_exit_code !== null) {
  throw new Error(
    "limited enforcement authority promotion must preserve null current_audit_exit_code"
  );
}
for (const exportName of permit.LIMITED_ENFORCEMENT_AUTHORITY_EXPORT_SET) {
  if (!(exportName in permit)) {
    throw new Error(
      `limited enforcement authority promotion export missing from permit index: ${exportName}`
    );
  }
}

process.stdout.write("audit limited enforcement authority promotion verified\n");
