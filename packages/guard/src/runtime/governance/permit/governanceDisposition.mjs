import schema from "./governance_disposition.schema.json" with { type: "json" };
import {
...
export const GOVERNANCE_DISPOSITION_KIND = "governance_disposition";
export const GOVERNANCE_DISPOSITION_VERSION = "v1";
export const GOVERNANCE_DISPOSITION_SCHEMA_ID = schema.$id;
export const GOVERNANCE_DISPOSITION_CONSUMER_SURFACE = "guard.audit";
export const GOVERNANCE_DISPOSITION_PRODUCER_SURFACE = "guard.audit";
export const GOVERNANCE_DISPOSITION_MODE = "explicit_opt_in";
...
function assertLinkageConsistency(gateResult, receipt, decisionRecord, outcomeBundle, applicationRecord) {
  if (receipt) {
    if (receipt.governance_receipt.outcome !== gateResult.permit_gate.decision) {
      throw new Error("governance disposition receipt linkage requires matching outcome");
    }
  ...
  if (decisionRecord) {
    if (decisionRecord.governance_decision.outcome !== gateResult.permit_gate.decision) {
      throw new Error("governance disposition decision record linkage requires matching outcome");
    }
  ...
  if (applicationRecord) {
    if (applicationRecord.governance_application.outcome !== gateResult.permit_gate.decision) {
      throw new Error("governance disposition application record linkage requires matching outcome");
    }
    if (applicationRecord.governance_application.applied_outcome !== gateResult.permit_gate.decision) {
      throw new Error(
        "governance disposition application record linkage requires matching applied outcome"
      );
    }
...
export function buildGovernanceDisposition({
  audit,
  ...
  if (bridgeContract.canonical_action_hash !== gateResult.canonical_action_hash) {
    throw new Error("governance disposition requires matching canonical action hashes");
  }
  if (receipt && receipt.canonical_action_hash !== gateResult.canonical_action_hash) {
    throw new Error("governance disposition receipt linkage requires matching canonical action hashes");
  }
  if (decisionRecord && decisionRecord.canonical_action_hash !== gateResult.canonical_action_hash) {
    throw new Error(
      "governance disposition decision record linkage requires matching canonical action hashes"
    );
  }
  if (outcomeBundle && outcomeBundle.canonical_action_hash !== gateResult.canonical_action_hash) {
    throw new Error(
      "governance disposition outcome bundle linkage requires matching canonical action hashes"
    );
  }
  if (
    applicationRecord &&
    applicationRecord.canonical_action_hash !== gateResult.canonical_action_hash
  ) {
    throw new Error(
      "governance disposition application record linkage requires matching canonical action hashes"
    );
  }
  assertLinkageConsistency(gateResult, receipt, decisionRecord, outcomeBundle, applicationRecord);
  ...
  const disposition = {
    ...
    traceability: {
      ...
      emitted_by: GOVERNANCE_DISPOSITION_EMITTER_SURFACE,
    },
    deterministic: true,
    enforcing: false,
  };
  ...
  if (receipt) {
    disposition.governance_receipt_linkage = {
  ...
  if (decisionRecord) {
    disposition.governance_decision_record_linkage = {
  ...
  if (outcomeBundle) {
    disposition.governance_outcome_bundle_linkage = {
  ...
  if (applicationRecord) {
    disposition.governance_application_record_linkage = {
...
export function validateGovernanceDisposition(artifact) {
  const errors = [];
...
export function assertValidGovernanceDisposition(artifact) {
  const result = validateGovernanceDisposition(artifact);
  if (result.ok) return artifact;

  const err = new Error(`governance_disposition failed validation: ${result.errors.join("; ")}`);
